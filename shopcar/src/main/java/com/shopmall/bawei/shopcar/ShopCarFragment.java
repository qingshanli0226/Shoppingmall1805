...
public class ShopCarFragment extends BaseFragment<ShopcarPresenterImpl, ShopcarContract.IShopcarView> implements ShopcarContract.IShopcarView, View.OnClickListener {
    ...
    protected void initPreseter() {
        ...
        shopcarAdapter.setShopcarPresenter(httpresetnter);
    }
    ...
    protected void initView(View inflate) {
        shopcarRv = (RecyclerView) inflate.findViewById(R.id.shopcarRv);

        allSelectCheckBox = (CheckBox) inflate.findViewById(R.id.allSelect);

        totalPriceTv = (TextView) inflate.findViewById(R.id.sumValue);

        normalLayout = (RelativeLayout) inflate.findViewById(R.id.normalLayout);
        editAllSelectCheckBox = (CheckBox) inflate.findViewById(R.id.allEditSelect);
        editLayout = (RelativeLayout) inflate.findViewById(R.id.editLayout);

        tvBianJi = (TextView)inflate.findViewById(R.id.tvBianJi);
        tvFinish = (TextView)inflate.findViewById(R.id.tvFinish);

        inflate.findViewById(R.id.deleteBtn).setOnClickListener(this);
        inflate.findViewById(R.id.payBtn).setOnClickListener(this);
        ...
        allSelectCheckBox.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (allSelectCheckBox.isChecked()){
                    newAllSelect = true;
                    httpresetnter.selectAllProduct(newAllSelect);
                }else {
                    newAllSelect = false;
                    httpresetnter.selectAllProduct(newAllSelect);
                }
...
//            httpresetnter.checkInventory(CacheManager.getInstance().getSelectedProductBeanList());
            //跳转到支付页面
            ...
            if (deleteShopcarBeanList.size()>0) {
                httpresetnter.deleteProducts(deleteShopcarBeanList);
            } else {
    ...
    public void onInventory(List<InventoryBean> inventoryBean) {
        ...
        if (checkInventoryIsEnough(inventoryBean)){
            //充足情况下，向服务端下订单
            httpresetnter.getOrderInfo(CacheManager.getInstance().getSelectedProductBeanList());
        }else {
            Toast.makeText(getContext(), ""+inventoryBean.get(0).getProductName()+"库存不充足", Toast.LENGTH_SHORT).show();
        }
