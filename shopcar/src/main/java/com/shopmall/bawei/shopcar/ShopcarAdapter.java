...
public class ShopcarAdapter extends BaseRvAdapter<ShopcarBean> {
    ...
    @Override
    protected void convert(final ShopcarBean itemData, BaseRvAdapter.BaseViewHolder baseViewHolder, final int position) {
        ImageView productImg = baseViewHolder.getView(R.id.productImage);
        Glide.with(baseViewHolder.itemView.getContext()).load(Constants.BASE_RESOURCE_IMAGE_URL+itemData.getUrl()).into(productImg);
